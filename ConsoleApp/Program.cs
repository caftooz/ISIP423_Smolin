...
Player player = new Player(35, 3, 6);

void end(string difficult)
{
    Thread.Sleep(1500);
    Console.Clear();
    if (player.hp <= 0) { Console.WriteLine("Вы проиграли!!"); return; }
...
void get_tools()
{
    ...
    switch (a)
    {
        case 0:
            ...
            break;
        case 1:
            Console.WriteLine("Вам выпал новый меч, взять? (1-да, 0-нет)");
...
{
    Console.WriteLine($"Враг: {enemy.name}");
    while (true)
    {
        if (!player.is_freeze)
        {
            ...
            switch (s)
            {
                ...
                case "2":
                    ...
                    Thread.Sleep(500);
                    player.Get_damage_in_def(enemy.Attack(), enemy.ignore_defence);
                    break;
            }
        }
        if (player.hp <= 0)
...
class Player
{
    ...
    public Player(double max_hp, double defence, double damage)
    {
        hp = max_hp;
        this.max_hp = max_hp;
    ...
    public void Get_damage_in_def(double dmg, bool ignore)
    {
        ...
        if (rand.NextDouble() > 0.40)
        {
            ...
            if (result > 0)
            {
                ...
                Console.WriteLine($"Персонаж получил {result} урона");
            }
            else
            {
                Console.WriteLine("Броня блокировала весь урон");
            }
        }
        else
        {
    ...
    {
        hp -= dmg;
        Console.WriteLine($"Персонаж получил {dmg} урона");
    }
...
class Enemy : Player
{
    ...
    public void Get_damage(double dmg)
    {
        ...
        {
            hp -= Double.Round(result, 3);
            Console.WriteLine($"Враг получил {result} урона");
        }
        else
        {
            Console.WriteLine("Враг защитился");
        }
        Thread.Sleep(1000);
    }
